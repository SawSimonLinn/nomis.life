          ...
          {rank && rank <= 3 && (
            ...
            >
              <Trophy
                className={cn(
                  "h-4 w-4",
                  rankIconClasses[rank as keyof typeof rankIconClasses]
                )}
              />
              <span>#{rank}</span>
            </div>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-2 sm:p-4 text-white">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p
                    className="text-xs sm:text-sm font-bold group-hover:underline"
                    style={{ textShadow: "1px 1px 2px rgba(0,0,0,0.7)" }}
                  >
                    {project.user.name}
                  </p>
                  <p
                    className="text-xs text-white/90 group-hover:underline"
                    style={{ textShadow: "1px 1px 2px rgba(0,0,0,0.7)" }}
                  >
              ...
              <div className="flex items-center justify-end gap-3 text-xs font-medium">
                {(project.reviewCount || 0) > 0 && (
                  <div
                    ...
                    title={`${project.reviewCount} reviews`}
                  >
