...
export default function ProjectForm({ project, onSubmitSuccess, user }: ProjectFormProps) {
  ...
  useEffect(() => {
    ...
    if (project) {
        ...
        const previews = project.imageIds?.map(id => getImageUrl(id)) || [];
    ...
    try {
      ...
      for (const value of data.imageValues) {
        if (typeof value === 'string') {
          ...
          const uploadedFile = await uploadImage(value);
      ...
      const payload = {
        ...
        techStack: data.techStack.split(',').map(tag => tag.trim()).filter(Boolean),
