...
function Stats() {
    const [stats, setStats] = useState({ totalProjects: 0, totalUsers: 0, totalViews: 0 });
    ...
    return (
        <div className="flex justify-center items-center gap-4 sm:gap-8 mt-8 text-sm sm:text-base text-muted-foreground">
            <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-foreground">{stats.totalUsers.toLocaleString()}+</span>
                <span>Developers</span>
            </div>
            <div className="flex items-center gap-2">
                 <GitBranch className="w-5 h-5 text-primary" />
                <span className="font-semibold text-foreground">{stats.totalProjects.toLocaleString()}+</span>
                <span>Projects</span>
            </div>
            <div className="flex items-center gap-2">
...
function ProjectPagination({ page, totalPages, onPageChange }: { page: number, totalPages: number, onPageChange: (page: number) => void }) {
    ...
    return (
        <Pagination>
            <PaginationContent>
                <PaginationItem>
                    <PaginationPrevious onClick={handlePrevious} className={page === 1 ? 'pointer-events-none opacity-50' : undefined} />
                </PaginationItem>
                {pageNumbers.map(p => (
                ...
                <PaginationItem>
                    <PaginationNext onClick={handleNext} className={page === totalPages ? 'pointer-events-none opacity-50' : undefined} />
                </PaginationItem>
            </PaginationContent>
        </Pagination>
    );
...
export default function Home() {
  ...
  const [popularSearches, setPopularSearches] = useState<string[]>(['AI Project', 'Portfolio Site', 'SaaS Platform', 'Mobile App', 'Data Analytics', 'Game Dev']);
  ...
  const fetchProjects = async (page: number, query: string) => {
    ...
    const projectsWithReviews = await getProjectsWithReviewCounts(response.projects);
  ...
  return (
    <>
      <Hero />
      <div id="projects" className="container mx-auto px-4 py-8 md:py-16 scroll-mt-20">
        <section className="mb-12 text-center">
            <h2 className="text-3xl font-bold font-headline mb-4">Explore the Community</h2>
            <p className="text-muted-foreground max-w-2xl mx-auto mb-6">
                Find projects by title, discover talented developers by name, or see what's trending.
            </p>
        ...
        <section>
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  ...
                  <ProjectCard key={project.$id} project={project} rank={searchQuery ? undefined : index + 1 + (currentPage - 1) * PROJECTS_PER_PAGE} />
                  ))}
              </div>
