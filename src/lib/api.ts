    ...
    const projects = await Promise.all(
      response.documents.map(async (doc) => {
        ...
        try {
          ...
          // Return a default/fallback user structure or null if the user is essential
          return null;
        }

        // The user document exists, so we can safely access its properties
        project.user = {
          id: project.userId,
          name: doc.developerName, // Use developerName from project doc for consistency
          username: userDoc.username, // This is the crucial fix
          avatarUrl:
...
export async function getAllProjects(
  ...
  try {
    ...
    const techStackQueries = searchTerms.map((term) =>
      Query.contains("techStack", term)
    );

    const queries = [
      Query.or([...titleQueries, ...developerNameQueries, ...techStackQueries]),
