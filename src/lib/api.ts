...
export async function getAppwriteUser() {
  try {
    ...
    if (!session || !session.userId) {
      console.warn("👻 Guest user — skipping account.get()");
      return null;
    }
    ...
    console.error("❌ Failed to fetch session/account:", err);
    return null;
  }
}
...
async function fetchGitHubProfile() {
  ...
  try {
    ...
    if (!session || !session.providerAccessToken) {
      // Guest or missing token — skip fetch
      return null;
    }
    ...
    const res = await fetch("https://api.github.com/user", {
      headers: {
        ...
        "User-Agent": "nomis.life-app",
      },
    });

    if (!res.ok) {
      console.error("❌ GitHub profile fetch failed:", res.statusText);
      return null;
    }
...
export async function mapAppwriteUserToUser(
  ...
  const userData: User = {
    ...
    avatarUrl: githubProfile?.avatarUrl || "/default-avatar.png",
    bio:
      githubProfile?.bio ||
      prefs.bio ||
      "This is a placeholder bio. Update it in your profile dashboard.",
  ...
  try {
    ...
    const updatePayload: Partial<User> = {
      ...
      careerPath:
        existingDoc.careerPath || userData.careerPath || "software_engineering",
      skills:
        existingDoc.skills && existingDoc.skills.length > 0
          ? existingDoc.skills
          : userData.skills,
      softSkills:
        existingDoc.softSkills && existingDoc.softSkills.length > 0
          ? existingDoc.softSkills
          : userData.softSkills,
    ...
    if (error.code === 404) {
      const realEmail = appwriteUser.email;
      ...
      response.documents.map(async (doc) => {
        ...
        try {
          ...
          project.user = {
            ...
            username: userDoc.username,
            avatarUrl:
              userDoc.avatarUrl || `https://github.com/${userDoc.username}.png`,
          };
        } catch (error) {
          console.warn(
            `Could not find user document for userId: ${project.userId}. Project: ${project.title}`
          );
        ...
        if (project.imageIds && project.imageIds.length > 0) {
          ...
          project.imageUrls = [
            "https://placehold.co/1920x1080?text=Project+Image",
          ];
        }
...
export async function getProjectsWithReviewCounts(
  projects: Project[]
): Promise<Project[]> {
...
export async function getAllProjects(
  ...
  try {
    ...
    const developerNameQueries = searchTerms.map((term) =>
      Query.search("developerName", term)
    );

    const queries = [
      Query.or([...titleQueries, ...developerNameQueries]),
...
export async function uploadImage(file: File): Promise<Models.File> {
  ...
  try {
    ...
    return await storage.createFile(storageBucketId, ID.unique(), file, [
      Permission.read(Role.any()),
...
export async function uploadResume(file: File): Promise<Models.File> {
  try {
    ...
    return await storage.createFile(RESUME_BUCKET_ID, ID.unique(), file, [
      Permission.read(Role.any()),
      Permission.update(Role.user(user.$id)),
      Permission.delete(Role.user(user.$id)),
    ]);
