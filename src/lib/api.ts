...
export async function mapAppwriteUserToUser(appwriteUser: Models.User<Models.Preferences>): Promise<User> {
    ...
    try {
        ...
        if (error.code === 404) {
             ...
             await databases.createDocument(DB_ID, USERS_COLLECTION_ID, appwriteUser.$id, createPayload);
        } else {
...
export async function getAllProjects(searchQuery: string = ''): Promise<Project[]> {
    ...
    try {
        ...
        const techStackQueries = searchTerms.map(term => Query.contains('techStack', term));
...
export async function updateProject(projectId: string, data: Partial<Omit<Project, 'user' | 'imageUrls' | '$id' | '$collectionId' | '$databaseId' | '$createdAt' | '$updatedAt' | '$permissions' | 'username'>>) {
    try {
